...
class TableDiff:
    ...
    def apply_d(self: TableDiff, table: Table):
        """
        Applies the diff onto the provided `table` with domination checks. 
        """
        for removed_entry in self.removed:
            table.entries.remove(removed_entry)
        
        for added_entry in self.added:
            table.insert_d(added_entry)


    def apply_ppd(self: TableDiff, table: Table):
        """
        Applies the diff onto the provided `table` with per parent domination checks. 
        """
        for removed_entry in self.removed:
